...
function computeTotals(items = [], taxPercent = 14) {
...
export async function getInvoiceById(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Invoice fetched", data: inv });
  } catch (err) {
...
export async function updateInvoiceById(req, res) {
  try {
    ...
    const updated = await Invoice.findByIdAndUpdate(existing._id, update, {
      new: true,
    });
    if (!updated) {
      return res
